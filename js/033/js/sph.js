...
//    const INIT_MAX        = new Vec3( 10.0, 30.0,  15.0);
    const INIT_MIN        = new Vec3(  0.0, 20.0,   0.0);
    const INIT_MAX        = new Vec3( 10.0, 50.0,   5.0);
...
//		p_ps.push(p);
    ...
    function advance(p_ps, p_nbr_map) {
        ...
        for (var i = 0; i < p_ps.length; i++) {
			...
			var dx = center_x - p_p.pos.x;
			var dy = center_y - p_p.pos.y;

			g.x = 9.8*dx/Math.sqrt(dx*dx + dy*dy);
			g.y = 9.8*dy/Math.sqrt(dx*dx + dy*dy);
	...
	this.get_around = function() {
        ...
        for (var i = 1; i < particles.length; i++) {
			if (particles[idx_y_top].pos.y < particles[i].pos.y) {
				idx_y_top = i;
			}
		}

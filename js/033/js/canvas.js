...
(function($) {
    canvasManager.canv = function(ctx, w, h) {
        ...
        this.draw = function() {
			...
			var outline = this.sph.get_around();
			
			for (var i = 1; i < outline.length; i++) {
				this.ctx.beginPath();
				this.ctx.moveTo(outline[i-1].pos.x*8,
								this.area.h-outline[i-1].pos.y*8);
				this.ctx.lineTo(outline[i].pos.x*8,
								this.area.h-outline[i].pos.y*8);
				this.ctx.stroke();
			}
            this.ctx.restore();
